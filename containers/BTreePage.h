...
public:
    ...
    // Collect: recorre el arbol y llena un vector con punteros a cada entry.
    // Uso interno del iterador de BTree.
    // forward: true = inorder, false = inorder invertido.
    void  Collect(vector<value_type*>& out, bool forward);
...
// Collect: recorrido inorder que junta punteros a cada entry en un vector.
template <typename Trait>
void BTreePage<Trait>::Collect(vector<value_type*>& out, bool forward) {
    if (forward) {
        for (size_t i = 0; i < m_KeyCount; ++i) {
            if (m_SubPages[i]) m_SubPages[i]->Collect(out, forward);
            out.push_back(&m_Keys[i]);
        }
        if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->Collect(out, forward);
    } else {
        if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->Collect(out, forward);
